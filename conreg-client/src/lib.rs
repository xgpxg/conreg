...
// Re-export feign macros when feature is enabled
#[cfg(feature = "feign")]
pub use conreg_feign_macro::{feign_client, get, post, put, delete, patch, path_variable, request_param, body};
...
mod tests {
    ...
    /// Test basic GET request with path parameter
        #[feign_client(service_id = "test-server2",url="https://baidu.com")]
        trait TestClient {
            #[get("/list")]
            async fn list(&self, id: i64,#[header(name = "alias")]token: String) -> Result<serde_json::Value, FeignError>;
        ...
        async fn test_feign() {
            //init().await;
            // This test verifies that the macro generates the Impl struct
            let client = TestClientImpl::new(crate::lb::LoadBalanceClient::new());
            println!("list: {:?}",client.list(1,"AAA".to_string()).await.unwrap());
        }
